...
%     figure(3)
%     subplot(4,4,i)
%     fin = length(GFP_norm-st);
%     boundedline(time(st:fin),GFP_norm(st:fin),stdev_norm(st:fin),'b-'); hold on
%     plot(time,Model_GFP_norm,'k--'); hold on
%     plot(ta_half,0.5,'o'); hold on
%     plot(td_half,0.5,'o'); hold on
%     plot(960,0,'ro'); hold on
%     set(gca,'FontSize',20)
%     xlim([0 time(end)])
%     ylim([0 1.3])    
...
%     ylim([0 3000])
    ...
    subplot(4,4,i)
    fin = length(GFP_norm);
    boundedline(time(1:fin),GFP_norm,stdev_norm,'b-'); hold on
    plot(time,Model_GFP_norm,'k--'); hold on
    plot(time(ind_low),0.5,'ro'); hold on
    plot(time(ind_high),0.5,'ko'); hold on
    
    plot(td_half+960,0.5,'o'); hold on
    plot(960,0,'ro'); hold on
    set(gca,'FontSize',20)
    xlim([0 time(end)])
    ylim([0 1.3])    
